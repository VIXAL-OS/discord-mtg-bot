        ...
        if a_type == "pass":
            ...
            if card_obj:
                ...
                ok, mana_by_color, any_color_mana = _simulate_cast_spend(
                    card_obj.mana_cost, mana_by_color, any_color_mana
                )
                if not ok:
                    ...
                    for _c in player.battlefield:
                        if _c.name.lower() == explicit_tgt_name.lower() and _c.is_creature():
                            print(f"[PLAN-VALIDATE] Rejected cast {card_name} — "
                                  f"hybridize-style spell targeting own creature ({_c.name})")
                            break
                    else:
                        pass  # for/else: not found, proceed
                    if any(_c.name.lower() == (explicit_tgt_name or '').lower() and _c.is_creature()
                           for _c in player.battlefield):
                        continue
    ...
    if game.attackers and not game.ended:
        ...
        while game.phase not in [Phase.MAIN2, Phase.END, Phase.CLEANUP] and not game.ended:
            if game.phase == Phase.DECLARE_ATTACKERS:
                ...
                engine.advance_phase(game)  # → DECLARE_BLOCKERS
                ...
                if not opponent.is_claude:
                    ...
                    engine.advance_phase(game)  # → COMBAT_DAMAGE
                ...
                engine.advance_phase(game)  # → COMBAT_END
                ...
                game.attackers = []
                game.blockers = {}
                engine.advance_phase(game)  # → MAIN2
            else:
                break
